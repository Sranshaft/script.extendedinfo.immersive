...
def HandleYouTubeVideoResults(results):
    ...
    for item in results:
            ...
            video = {'Thumb': thumb,
                     'youtube_id': videoid,
                     'Play': 'plugin://script.extendedinfo.immersive/?info=youtubevideo&&id=%s' % videoid,
                     'Path': 'plugin://script.extendedinfo.immersive/?info=youtubevideo&&id=%s' % videoid,
    ...
    if results:
        for item in results["feed"]["entry"]:
            video = {'Thumb': item["media$group"]["media$thumbnail"][2]["url"],
                     ...
                     'Plot': item["media$group"]["media$description"]["$t"],
                     'Title': item["title"]["$t"],
                     'Author': item["author"][0]["name"]["$t"],
