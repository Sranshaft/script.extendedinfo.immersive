...
class SlideShow(xbmcgui.WindowXMLDialog):
    ...
    def onInit(self):
        if self.imagelist:
            ...
            listitem = {"Label": self.image, "Thumb": self.image}
...
def GetSimilarArtistsInLibrary(artistid):
    ...
    for (count, simi_artist) in enumerate(simi_artists):
        for (count, xbmc_artist) in enumerate(xbmc_artists["result"]["artists"]):
            log('Checking %s against %s for similar artists' % (xbmc_artist['artist'],simi_artist['name']))
...
def GetSimilarMoviesInLibrary(dbid):
...
def read_from_file(path=""):
    ...
    if xbmcvfs.exists(path):
        cachefile = open(path)
        response = simplejson.load(cachefile)
        return response
    else:
        return False
    ...
    if json_query["result"]["limits"]["total"] > 0:
        for fav in json_query["result"]["favourites"]:
            ...
            newitem = {'Label': fav["title"],
                       ...
                       'Type': fav["type"],
                       'Builtin': path,
                       'Path': "plugin://script.extendedinfo.immersive/?info=action&&id=" + path}
...
def GetIconPanel(number):
    ...
    for i in range(1, 6):
        newitem = {'Label': xbmc.getInfoLabel("Skin.String(IconPanelItem" + str(i + offset) + ".Label)").decode("utf-8"),
                   'Path': "plugin://script.extendedinfo.immersive/?info=action&&id=" + xbmc.getInfoLabel("Skin.String(IconPanelItem" + str(i + offset) + ".Path)").decode("utf-8"),
...
def GetWeatherImages():
    ...
    for i in range(1, 6):
        newitem = {'Label': "bla",
                   'Path': "plugin://script.extendedinfo.immersive/?info=action&&id=SetFocus(22222)",
...
def ConvertYoutubeURL(string):
    if 'youtube.com/v' in string:
        ...
        for id in vid_ids:
            convertedstring = 'plugin://script.extendedinfo.immersive/?info=youtubevideo&&id=%s' % id
...
def passListToSkin(name="", data=None, prefix="", controlwindow=None, controlnumber=None, handle=None, limit=False, debug=False):
    ...
    if controlnumber is "plugin":
        ...
        log("creating listitems for list with id " + str(controlnumber))
        xbmc.sleep(200)
